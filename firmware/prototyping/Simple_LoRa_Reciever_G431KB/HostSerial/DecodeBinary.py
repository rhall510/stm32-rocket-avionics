...
# Create a large figure with 7 stacked subplots that share the X-axis (time)
fig, axs = plt.subplots(4, 2, figsize=(12, 12), sharex=True)
fig.suptitle("Avionics Telemetry Data Over Time", fontsize=16)

# Helper trick: zip(*list) transposes our list of rows into separate columns
# e.g., [[t1, x1], [t2, x2]] becomes (t1, t2), (x1, x2)
...
# 7. GPS (Plotting Altitude and Speed on dual Y-axes)
if gps:
    t, lat, long, alt, spd, sats, fix = zip(*gps)
    
    # Primary axis for Altitude
    axs[3, 0].plot(t, alt, label='Altitude', color='blue')
    axs[3, 0].set_ylabel("Altitude", color='blue')
    ...
    axs[3, 0].set_title("GPS Altitude & Speed")
